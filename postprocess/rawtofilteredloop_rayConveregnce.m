%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The script loops over multiple wall distances (rayCounts) for one DNS case
...
% Distances (heights)
fileName = "rayconvergence.csv";

%% Don't change below here
S = readtable(fileName);
rayCounts = string(S.RayCount);

baseRayTraceDir = uigetdir(matlabroot, 'Select folder with raytraced cases');

[parentDir, ~, ~] = fileparts(baseRayTraceDir);
baseFilteredDir = fullfile(parentDir, caseTag + '_convergencefiltered');


for d = 18:numel(rayCounts)
    rayCount = rayCounts(d);
    fprintf('Processing %s...\n', rayCount);

    % Finding subfolders:
    rayTraceDir = fullfile(baseRayTraceDir, ...
        caseTag + "_raytraced_" + rayCount);
    
    % Building subfolders:
    filteredDir = fullfile(baseFilteredDir, ...
        caseTag + "_raytraced_filtered_extra_" + rayCount);
    ...
    for k = 1:numel(files)
        ...
        newName = [baseName '_filtered_simple_extra' ext];
