...
inDir = 'D:\CorrelationAnalysis_rayTrace\RayTrace_500SAMPLED_B1024_D3pi';
outputFolder = 'D:\CorrelationAnalysis_connected\RayTrace_500SAMPLED_B1024_D3pi';
...
for k = 1:numel(files)
    filePath = fullfile(inDir, files(k).name);
    inFile = files(k);
    ...
    % LOAD
    S = load(filePath);
    img = double(S.screen.image);   % change this if your field name differs
    ...
    if k == 250
        ...
        title(sprintf('Smoothed Image for File: %s', files(k).name));
    end
    ...
    if k == 250
        ...
        title(sprintf('Contrast Enhanced Result File: %s', files(k).name));
    end
    ...
    % outImg = fullfile(outputFolder, baseName + "_smoothed.jpg");
    % %imwrite(X1tBig, outImg);

    %fprintf("Saved %d/%d: %s\n", k, numel(files), outImg);
    ...
    if numel(order1) < params.areastokeep
      warning('Only %d elements available, requested %d', numel(order1), params.areastokeep);
    end
    ...
    if k == 250
        figure;
        imagesc(X1tBig); axis image off; colormap gray;
        title(sprintf('Largest Warm Areas File: %s', files(k).name));
    end
